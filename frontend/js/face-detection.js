...
async function loadModels() {
  ...
  try {
    ...
    console.warn(
      "Nepodařilo se nastavit WebGL backend, bežím na:",
      tf.getBackend(),
    );
  }
      ...
      if (detections) {
        ...
        if (USE_SMOOTHING) {
          ...
          const step = (current, next) =>
            Math.abs(next - current) > maxStep
              ? current + Math.sign(next - current) * maxStep
              : next;
          smX = step(smX, nextX);
          smZ = step(smZ, nextZ);
          outX = smX;
        ...
        if (++drawCounter % 2 === 0) {
          //kreslí každý 2 frame pro úsporu
          ctx.clearRect(0, 0, canvas.width, canvas.height);
          const displaySize = {
